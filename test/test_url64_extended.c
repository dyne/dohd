...
static int test_null_input(void) {
    int check = dohd_url64_check(NULL, 0);
...
static int test_single_char(void) {
    ...
    /* Single valid char */
    int check = dohd_url64_check("A", 1);
    TEST_ASSERT(check == 1, "Single valid char check returns 1");

    len = dohd_url64_decode("A", 1, out, sizeof(out));
...
static int test_url64_chars(void) {
    ...
    check = dohd_url64_check("AAA-", 4);  /* URL-safe minus */
    TEST_ASSERT(check == 4, "URL-safe '-' is valid");

    check = dohd_url64_check("AAA_", 4);  /* URL-safe underscore */
...
static int test_dns_query(void) {
    ...
    int check = dohd_url64_check(dns_b64, strlen(dns_b64));
    TEST_ASSERT(check == 39, "DNS query check returns correct length");

    len = dohd_url64_decode(dns_b64, strlen(dns_b64), out, sizeof(out));
...
static int test_all_valid_chars(void) {
    ...
    int check = dohd_url64_check(all_valid, strlen(all_valid));
...
static int test_long_input(void) {
    ...
    int check = dohd_url64_check(long_input, strlen(long_input));
    TEST_ASSERT(check == 2000, "Long input check returns correct length");

    int len = dohd_url64_decode(long_input, strlen(long_input), out, sizeof(out));
...
static int test_padding_scenarios(void) {
    ...
    len = dohd_url64_decode("QQ", 2, out, sizeof(out));
    TEST_ASSERT(len == 1, "2 char input -> 1 byte output");

    /* 3 chars = 2 bytes output */
    len = dohd_url64_decode("QUE", 3, out, sizeof(out));
    TEST_ASSERT(len == 2, "3 char input -> 2 bytes output");

    /* 4 chars = 3 bytes output */
    len = dohd_url64_decode("QUFB", 4, out, sizeof(out));
    TEST_ASSERT(len == 3, "4 char input -> 3 bytes output");

    return 1;
}

static int test_decode_capacity_limit(void) {
    uint8_t out[2];
    int len = dohd_url64_decode("QUFB", 4, out, sizeof(out));
    TEST_ASSERT(len == -1, "decode fails when destination capacity is too small");
    return 1;
}
